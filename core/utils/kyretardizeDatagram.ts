import { Buffer } from 'buffer';

const sampEncryptionTable = [
  
];
/**
 * Crypt SA:MP buffer for send to the server.
 * @param buffer 
 * @param length 
 * @param port 
 * @returns 
 */
export async function kyretardizeDatagram(buffer: Buffer, length: number, port: number): Promise<Buffer> {
  let xorPort = false;
  let checksum = 0;

  const encryptedBuffer = Buffer.alloc(length);
  const bPort = port ^ 0xCC;

  for (let i = 0; i < length; i++) {
    checksum ^= buffer[i] & 0xAA;
  }
  encryptedBuffer[0] = checksum;

  for (let i = 1; i < length; i++) {
    encryptedBuffer[i] = buffer[i - 1];
  }

  for (let i = 1; i <= length; i++) {
    const crypt = sampEncryptionTable[encryptedBuffer[i]];
    encryptedBuffer[i] = crypt;
    if (xorPort) {
      encryptedBuffer[i] = bPort ^ crypt;
    }
    xorPort = !xorPort;
  }

  return encryptedBuffer;
}